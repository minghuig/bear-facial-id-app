#!/usr/bin/env python3
"""Verify private checkpoint hashes, then upload only the three required files."""
...
p.add_argument('--prototype', type=pathlib.Path, default=root.parent/'bear-id')
a=p.parse_args()
...
for name,expected in HASHES.items():
    path=a.prototype/'artifacts'/('detectors' if name.startswith('bear_head') else '')/name
    h=hashlib.sha256()

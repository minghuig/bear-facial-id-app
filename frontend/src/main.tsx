...
type Head={id:string;index:number;body_index:number|null;detector_score:number|null;box:number[];crop_url:string;crop_review_state:'pending'|'accepted'|'rejected';recognition_state:string;review_state:string;bear_id:string|null;error:string|null;suggestions:{id:string;created_at:string;gallery_revision:number;candidates:Candidate[]}[]};
...
function App({identity}:{identity:Identity}){
 ...
 const similar=useQuery({queryKey:[identity.org_id,'comparison',head?.id],queryFn:()=>api<Matches>(`/api/heads/${head!.id}/matches`),enabled:!!head&&head.crop_review_state==='accepted'&&head.recognition_state==='complete'&&!['ignored','unusable'].includes(head.review_state)&&!comparison,refetchInterval:comparison?false:5000});
 ...
 function curate(state:'accepted'|'rejected'){if(head)void act(()=>api(`/api/heads/${head.id}/crop-review`,'POST',{state}));}
 const recognizing=!!detail.data?.heads.some(h=>h.crop_review_state==='accepted'&&['queued','running'].includes(h.recognition_state));
    ...
    {tab===0?<div className="photo-workspace">
     ...
     {detail.data&&<section className="photo-detail" aria-label="Photo review">
      <div className="photo-heading">
       ...
       <Typography variant="body2" color="text.secondary">{detail.data.detection_state==='complete'?(detail.data.heads.length?`${detail.data.heads.length} detected head crop${detail.data.heads.length===1?'':'s'} · approve usable crops before recognition`:'No head detected'):detail.data.detection_state==='failed'?'Detection failed':'Detecting bears and heads…'}</Typography></div>
      ...
      <div className="review-grid">
       <div className="photo-column">
        ...
        {!!detail.data.heads.length&&health.data&&<div className="recognition-bar">
         <Typography variant="body2" color="text.secondary">{health.data.auto_recognize===false?'Approve each usable head crop, then run recognition on accepted sightings.':'Approve each usable head crop. Recognition starts automatically after approval.'}</Typography>
         {health.data.auto_recognize===false&&<Button startIcon={recognizing?<CircularProgress size={16} color="inherit"/>:undefined} aria-busy={recognizing} disabled={busy||recognizing||!detail.data.heads.some(h=>h.crop_review_state==='accepted'&&['not_requested','failed'].includes(h.recognition_state)&&!['ignored','unusable'].includes(h.review_state))} variant="contained" onClick={()=>act(()=>api(`/api/photos/${selected}/recognize`,'POST'))}>{recognizing?'Recognizing…':'Run recognition'}</Button>}
        </div>}
        {detail.data.jobs.filter(j=>j.error&&j.state!=='superseded'&&!(['detection','body_detection','head_detection'].includes(j.stage)&&detail.data.detection_state==='complete')).map(j=><Alert key={j.id} severity={j.state==='failed'?'error':'warning'}>{j.stage.replaceAll('_',' ')}: {j.error} (attempts: {j.attempts}, {j.state}) {j.state==='failed'&&['detection','body_detection','head_detection'].includes(j.stage)&&<Button disabled={busy} onClick={()=>act(()=>api(`/api/jobs/${j.id}/retry`,'POST'))}>Retry detection</Button>}</Alert>)}
       </div>
       {head&&<Card className="review-panel"><CardContent sx={{p:2,'&:last-child':{pb:2}}}><Stack spacing={1.5}>
        ...
        <div className="head-summary">
         ...
         <div><Chip size="small" variant="outlined" label={`Crop: ${head.crop_review_state}`}/>{head.crop_review_state==='accepted'&&<><Chip size="small" variant="outlined" sx={{ml:.5}} label={head.review_state.replaceAll('_',' ')}/><Typography variant="body2" color="text.secondary" sx={{mt:1}}>Recognition: {head.recognition_state==='not_requested'?'not run':head.recognition_state.replaceAll('_',' ')}</Typography>{head.bear_id&&<Typography variant="body2" sx={{mt:.5,overflowWrap:'anywhere'}}>{label(bears.data?.find(b=>b.id===head.bear_id)||{id:head.bear_id,name:null})}</Typography>}</>}</div>
        </div>
        {head.crop_review_state!=='accepted'?<div className="panel-section"><Typography component="h3" variant="subtitle2">Is this a usable bear-head crop?</Typography><Typography variant="body2" color="text.secondary" sx={{my:1}}>{head.crop_review_state==='rejected'?'This crop is excluded from recognition and matching. You can restore it if needed.':'Approve crops that clearly frame a bear head. Reject false detections, badly clipped heads, and unusable crops.'}</Typography><Stack direction="row" spacing={1}><Button variant="contained" disabled={busy} onClick={()=>curate('accepted')}>{head.crop_review_state==='rejected'?'Restore & approve':'Approve crop'}</Button>{head.crop_review_state==='pending'&&<Button color="error" disabled={busy} onClick={()=>curate('rejected')}>Reject crop</Button>}</Stack></div>:<>
        ...
        <Button color="error" disabled={busy||head.review_state==='confirmed'} onClick={()=>curate('rejected')}>Reject this crop</Button>
        </>}

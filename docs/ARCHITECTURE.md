...
Google sign-in creates revocable server-side sessions. Invited members can use the Internal Testing and/or McNeil organization, and requests are scoped to the selected organization. One configured Google `sub` grants access to member Settings; every management API request checks that identity, while ordinary members retain equal access to their library's photos and reviews. Removing a membership revokes sessions scoped to that organization. PostgreSQL persists users, memberships, sessions, photos, head observations, embeddings, bear identities, reviews, suggestion snapshots, and job history. Original photos and derived crops/previews stay in private S3; browser image requests pass through the authenticated API. The current release process backs up PostgreSQL and runs Alembic migrations without clearing the library. See [AWS operations](AWS_RELEASE.md) for the current release path.

...
2. Initialize an empty Aurora database using reviewed migrations. Seed Internal Testing and McNeil plus the intended invitation configuration, including the owner's invitation; do not copy historical user/session records. Verify both libraries are empty and isolated.
3. Configure fresh application storage and scoped runtime credentials. Carry the verified `OWNER_GOOGLE_SUB` into the new runtime configuration outside the database; record it before discarding the old database, then verify the owner can sign in, receive a new user row, and manage invitations after cutover. Reuse the verified model artifacts and pinned container images as appropriate; model files are dependencies, not disposable library data. Clear incompatible old browser sessions and OAuth cookies.

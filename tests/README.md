...
Coverage includes persisted detection pause, no-head and multi-head outcomes, exact-byte deduplication, explicit recognition, empty gallery, later-photo matching, correction and rename history, immutable suggestions, exclusions, finite normalized embedding checks, partial failures, bounded retry, stale tokens, heartbeat limits, duplicate result delivery, concurrent claims and recognition requests, worker credentials and pipeline separation.
